...
export default function ForegroundAddPage() {
  ...
  const addForeground = useForegroundStore((state) => state.addForeground);
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      if (error) {
        console.error(`전경 이미지 업로드 실패:`, error.message);
        alert(`전경 이미지 업로드 중 오류가 발생했습니다.`);
        return null;
      }

      const { data } = supabase.storage.from('foregrounds').getPublicUrl(filePath);
  ...
  const handleAddForeground = () => async () => {
    if (!title.trim()) {
      alert('제목을 입력해 주세요.');
      return;
    }
    if (!mediaFile) {
      alert('전경 이미지를 업로드해 주세요.');
      return;
    }

    try {
      const mediaUrl = await uploadFile(mediaFile);
      await addForeground(schoolId, title, mediaUrl);

      alert('전경 이미지가 성공적으로 추가되었습니다.');
      router.replace(`/admin/${schoolId}/introduction?tab=foreground`);
    } catch (error) {
      console.error('전경 이미지 추가 중 오류가 발생했습니다. : ', error);
      alert('전경 이미지 추가 중 오류가 발생했습니다. 다시 시도해주세요.');
      return;
    }
  };
      ...
      <section className="relative bg-white w-full p-4 md:p-10 border border-gray-200 rounded-xl shadow-dropdown md:w-[1080px] md:min-h-[753px]">
        <header className="relative flex flex-col justify-start gap-1">
          <ol className="flex justify-start items-center">
            <li>
              <Badge active>전경 추가</Badge>
            </li>
          </ol>
          <h3 className="text-24 text-gray-900 font-semibold">전경 추가</h3>
