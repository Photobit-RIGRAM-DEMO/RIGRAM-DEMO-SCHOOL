...
export default function Executive({ mode }: ExecutiveProps) {
  ...
  const handleDeleteButton = async () => {
    ...
    if (!confirm(`${selectedExecutiveIds.length}명의 교직원을 정말 삭제하시겠습니까?`)) return;
  ...
  return (
    <>
      <div className="relative flex flex-col gap-1.5 mb-4 md:mb-6">
        <h3 className="text-18 font-semibold md:text-24 md:text-gray-900">교직원</h3>
        <div className="flex justify-start items-center gap-1">
          <ImageIcon className="w-4 h-4" aria-hidden="true" />
          <span>{executives.length}명의 교직원</span>
        </div>
        {mode === 'admin' && (
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide md:max-h-[520px] ">
        {executives.length === 0 && <p>등록된 교직원이 없습니다.</p>}
            ...
            return (
              ...
              >
                ...
                <figcaption className="flex flex-col gap-1">
                  <h3 className="text-18 font-semibold text-gray-800 truncate">
                    {executive.name}
                  </h3>
                  <span className="text-16 text-gray-600">
                    {getRoleLabel(executive.position)}
                    {(executive.position === "teacher") &&` (${executive.role})`}
                    {(executive.position === "staff") &&` (${executive.role})`}
                  </span>
                </figcaption>
